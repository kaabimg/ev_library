...
class action_t {
public:
    virtual event_t exec(command_queue_t&,const std::vector<event_t> &wait_list = std::vector<event_t>())const = 0;
};


class write_buffer_action_t :
        public action_t,
        protected buffuer_action_data_t
{
public:
    write_buffer_action_t(const buffer_t & buffer,void * data,size_t size);
    event_t exec(command_queue_t&,const std::vector<event_t> &wait_list= std::vector<event_t>())const final;
};




class read_buffer_action_t :
        public action_t,
        protected buffuer_action_data_t
{
public:
