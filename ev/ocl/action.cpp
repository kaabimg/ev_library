                                    ...
                                    const std::vector<event_t>& wait_list) const
{
    ...
    cl_int status =
        clEnqueueWriteBuffer(cq.cl_object(), m_buffer,
                             CL_FALSE,  // nono blocking
                             0,         // offset
                             m_size, m_data, wait_list.size(),
                             wait_list.empty() ? nullptr : wait_events, &event);
                                   ...
                                   const std::vector<event_t>& wait_list) const
{
    ...
    cl_int status =
        clEnqueueReadBuffer(cq.cl_object(), m_buffer,
                            CL_FALSE,  // blocking
                            0, m_size, m_data, wait_list.size(),
                            wait_list.empty() ? nullptr : wait_events, &event);
                                   ...
                                   const std::vector<event_t>& wait_list) const
{
    ...
    check_status(clEnqueueNDRangeKernel(
        cq.cl_object(), m_kernel, m_range.dimension, nullptr, m_range.size,
        nullptr, wait_list.size(), wait_list.empty() ? nullptr : wait_events,
        &event));
