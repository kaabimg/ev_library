...
struct conditional_scope_exit_proactor_t {
    ...
    ~conditional_scope_exit_proactor_t(){if(cond())action();}
};
...
struct scope_exit_condition_t {
    ...
    conditional_scope_exit_proactor_t<Cond,Func> operator << (Func && f){
        return conditional_scope_exit_proactor_t<Cond,Func>(std::move(cond),std::forward<Func>(f));
    }
...
inline auto make_scope_exit_condition(auto && c){
    ...
    return scope_exit_condition_t<condition_type>(std::forward<condition_type>(c));
}
...
struct exceptions_counter_t {
    ...
    bool has_new_exceptions()const{return std::uncaught_exceptions() != value;}
};
...
#define ev_on_scope_exit \
    auto EV_ANONYMOUS_VARIABLE(_ev_on_scope_exit) = ::ev::detail::scope_exit_builder_t() << [&]()noexcept
...
#define ev_conditional_scope_exit(cond)\
    auto EV_ANONYMOUS_VARIABLE(_ev_conditional_scope_exit) = ::ev::detail::make_scope_exit_condition \
    ([&]() noexcept {return (cond);} ) << [&]()noexcept
...
#define ev_on_scope_exit_with_exception  \
    auto EV_ANONYMOUS_VARIABLE(_ev_scope_exit) = ev::detail::exception_proactor_builder_t<true>() << [&]  () noexcept
...
#define ev_on_scope_exit_without_exception  \
    auto EV_ANONYMOUS_VARIABLE(_ev_scope_exit) = ev::detail::exception_proactor_builder_t<false>() << [&] () noexcept
