...
struct property_notifier {
    ...
    {
        if (another._observers) _observers = new std::vector<observer>(*another._observers);
    }
...
struct observer_pack {
    ...
    {
        ev::for_each(_observers, [&p](auto& observer) { observer(p); });
    }
