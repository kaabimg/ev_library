...
constexpr void for_each_tuple(auto&& t, auto&& f)
{
    ...
    if constexpr (index + 1 < max)
        detail::for_each_tuple<index + 1, max>(std::forward<decltype(t)>(t),
...
constexpr void for_each_tuple2(auto&& t, auto&& f)
{
    ...
    if constexpr (index + 1 < max)
        detail::for_each_tuple2<index + 1, max>(std::forward<decltype(t)>(t),
                                                std::forward<decltype(f)>(f));
}
}
...
inline constexpr void for_each(std::tuple<Ts...>& tuple, F&& f)
{
    ...
    if constexpr (tuple_size > 0) {
        detail::for_each_tuple<0, tuple_size>(tuple, std::forward<F>(f));
    }
}
...
inline constexpr void for_each(std::tuple<Ts...>&& tuple, F&& f)
{
    constexpr size_t tuple_size =
        std::tuple_size<typename std::decay<std::tuple<Ts...>>::type>::value;
    if constexpr (tuple_size > 0) {
        detail::for_each_tuple<0, tuple_size>(std::move(tuple), std::forward<F>(f));
    }
}
...
inline constexpr void for_each2(const std::tuple<Ts...>& tuple, F&& f)
{
    constexpr size_t tuple_size =
        std::tuple_size<typename std::decay<std::tuple<Ts...>>::type>::value;
    if constexpr (tuple_size > 0) {
        detail::for_each_tuple2<0, tuple_size>(tuple, std::forward<F>(f));
    }
}
...
inline constexpr void for_each2(std::tuple<Ts...>&& tuple, F&& f)
{
    constexpr size_t tuple_size =
        std::tuple_size<typename std::decay<std::tuple<Ts...>>::type>::value;
    if constexpr (tuple_size > 0) {
        detail::for_each_tuple2<0, tuple_size>(std::move(tuple), std::forward<F>(f));
    }
}
}
