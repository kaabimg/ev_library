...
const x3::rule<struct unary_expression_type, ast::operand_t> unary_expression =
    "unary_expression";

const x3::rule<struct primary_expression_type, ast::operand_t>
    primary_expression = "primary_expression";

const x3::rule<struct function_call_type, ast::function_call_t> function_call =
    "function_call";

const x3::rule<struct expression_type, ast::expression_t> expression =
    "expression";

const x3::rule<struct function_declaration_type, ast::function_declaration_t>
    function_declaration = "function_declaration";

const x3::rule<struct anonymous_function_declaration_type,
               ast::anonymous_function_declaration_t>
...
const x3::rule<struct error_handler_t, ast::statement_t> statement =
    "statement";
...
auto const primary_expression_def =
    number | function_call | variable | ('(' >> expression >> ')');

auto const expression_def = additive_expression;

auto const function_declaration_def = identifier >> '(' >>
...
auto const statement_def =
    function_declaration | anonymous_function_declaration | expression;
...
inline bool init_operators() {
    ...
    multiply_operator.add("*", ast::operator_type_e::times)(
        "/", ast::operator_type_e::divide);

    unary_operator.add("+", ast::operator_type_e::positive)(
...
parser_result_t parser_t::parse(const std::string& line) {
    ...
    if (begin == end) {
        result.statement =
            std::make_shared<ast::statement_t>(std::move(statement));
        return result;
    }
