...
bool value_t::is_number() const
{
    switch (type().kind())
    {
        ...
        case type_kind_e::r64: return true;
...
value_t value_t::operator*(const value_t& another)
{
    ...
    if (values.first.type().is_integer()) {
        return create_object<value_t>(
            d->context, d->context->builder.CreateMul(values.first->data,
                                                      values.second->data));
    }

    return create_object<value_t>(
        d->context, d->context->builder.CreateFMul(values.first->data,
                                                   values.second->data));
}
...
value_t value_t::cast_to(const type_t& dest_type) const
{
    ...
    if (iter != cast_instructions.end()) {
        return create_object<value_t>(
            d->context, d->context->builder.CreateCast(iter->second, d->data,
                                                       dest_type.data()));
    }
    // TODO error
    return *this;
