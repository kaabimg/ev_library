...
struct compile_exception_t : std::exception {
    ...
    compile_exception_t(const std::string& what,const text_position_t& where = text_position_t()):
...
#define EV_VM_EXCEPTION(name,t) \
    name(const std::string& what,const text_position_t& where= text_position_t())\
