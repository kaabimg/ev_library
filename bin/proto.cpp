...
void transform_opencl_gpu()
{
    ...
    try {
        ...
        {
            std::vector<ocl::device_t> gpu_devices = platform.get_devices(ocl::device_type_e::gpu);
            ...
            ocl::buffer_t data_buffer = context.new_buffer<float>(data_size,ocl::memory_flags_e::read_write);

            ocl::program_t program = context.new_program_from_file(
                        "/home/ghaith/data/development/workspace/examples/opencl/vector_add.cl"
                        );
...
void transform_opencl_cpu()
{
    ...
    try {
        for (ocl::platform_t platform : ocl::get_platfomrs())
        {
            std::vector<ocl::device_t> gpu_devices = platform.get_devices(ocl::device_type_e::cpu);
